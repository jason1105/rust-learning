...
fn main() {
    ...
    impl<T> Deref for MyBox<T> {
        ...
        fn deref(&self) -> &Self::Target {
            // Self::Target means Deref::Target, the type of return value must be a reference. so add &.
    ...
    // We have to add lifetime when we impl Cons use reference
    {
        #[derive(Debug)]
        enum List2<'a> {
            Cons(i32, Box<&'a List2<'a>>),
            Nil,
        }

        use List2::{Cons, Nil};

        let x = Cons(10, Box::new(&Nil));
        let a = Cons(5, Box::new(&x));
    ...
    // while using Rc, compiler drop value depending on count number of Rc instead of lifetime
    // Code seems more elegant.
    {
        #[derive(Debug)]
        enum List3 {
            Cons(i32, Rc<List3>),
...

    {
        ...
        struct S {
            greet: Cell<&'static str>,
        }

        let s = S {
            greet: Cell::new("hello"),
        };

        let mut v = s.greet.get();
        s.greet.set("hello, world");
    }
}
